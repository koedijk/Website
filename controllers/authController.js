...
// Fetch fresh enemy faction data
...
// Merge claims into live enemy data
    ...
    db.query(
      ...
      (err) => {
        if (err) {
          ...
          return res.status(500).send('Failed to claim');
        }
        ...
        db.query(
          ...
          (err, results) => {
            if (err || results.length === 0) {
              console.error('Fetch after claim error:', err);
